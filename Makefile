...
read-mysql-up:
	...
	docker run --name read-mysql-2 -p 33063:3306 -e MYSQL_ROOT_PASSWORD=1234 -d mysql:8.0.28
...
READ_DB_URL_2="mysql://root:1234@(127.0.0.1:33063)/go_example?parseTime=true&charset=utf8mb4&collation=utf8mb4_unicode_ci&loc=UTC"

## migrate up or down
db-migrate-up:
	migrate -database ${WRITE_DB_URL} -path db/migrations -verbose up
	migrate -database ${READ_DB_URL} -path db/migrations -verbose up
	migrate -database ${READ_DB_URL_2} -path db/migrations -verbose up
db-migrate-down:
	...
	migrate -database ${READ_DB_URL_2} -path db/migrations -verbose down
